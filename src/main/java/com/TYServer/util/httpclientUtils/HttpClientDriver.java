package com.TYServer.util.httpclientUtils;
...
/**
 * Created by zhanghuibin on 2016/12/21.
 *
 */
public class HttpClientDriver {
    ...
    public static CloseableHttpClient httpClient01;
    ...
    public static String httpGet(String url,HashMap<String, String> header){
        HttpGet get = new HttpGet(url);
        get.addHeader(new BasicHeader("Cookie","JSESSIONID=B6FF25530B16AB46CA77B08129FECFB3"));
        get.addHeader(new BasicHeader("Cookie","company=www.tingyun.com"));
        String result = null;
        if (!header.isEmpty() || null != header) {
            Set<?> entrySet = header.entrySet();
            for (Iterator<?> itor = entrySet.iterator(); itor.hasNext();) {
    ...
    private static String executeRequest(HttpUriRequest req){
        ...
        try{
            ...
            System.out.println("接口请求返回状态码：" +httpResponse.getStatusLine().getStatusCode());
